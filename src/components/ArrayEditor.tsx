...
export const ArrayEditor = ({ value = [], onChange, item, context }: Props) => {
  ...
  const [advancedMetrics, setAdvancedMetrics] = useState<Record<number, boolean>>({});
        ...
        <div key={index} style={{ borderBottom: '1px solid rgba(204, 204, 220, 0.2)', marginBottom: 8, paddingBottom: 8 }}>
          {kind === 'metrics' && (
            <>
              ...
              <Button size="sm" variant="secondary" onClick={() => setAdvancedMetrics((current) => ({ ...current, [index]: !current[index] }))}>
                {advancedMetrics[index] || (entry as MetricMapping).showTrend || (entry as MetricMapping).trendField || ((entry as MetricMapping).valueMappings ?? []).length > 0 || ((entry as MetricMapping).thresholds ?? []).length > 0 ? 'Hide advanced metric options' : 'Show advanced metric options'}
              </Button>
              {(advancedMetrics[index] || (entry as MetricMapping).showTrend || (entry as MetricMapping).trendField || ((entry as MetricMapping).valueMappings ?? []).length > 0 || ((entry as MetricMapping).thresholds ?? []).length > 0) && <>
